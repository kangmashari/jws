...
void loop()
  { 
    ...
    anim_masjid(1);
   anim_DT(2);                                                   // addr: 1 show date time
   ...
   dwMrq(msgPuasa(hd_puasa,ty_puasa),50,0,5);                  // addr: 5 show Remander Puasa
    ...
    anim_JG(8);                                                 // addr: 7 show date time    
    ...
    drawJumatan(103); //Message Sholat jumat       // addr: 203 show Message Jum'at
...
//  if(RunFinish==3) {RunSel = 3; RunFinish =0;}
    ...
    if(RunFinish==10) {RunSel = 11;RunFinish =0;}                      //after anim 4 set anim 5
...
void update_All_data()
 ...
 check_puasa();                                              // check jadwal Puasa Besok
  ...
  if ((floatnow > (float)21) or (floatnow < (float)3) )    {setBrightness(7);}
    ...
    for(int i=0; i <=7; i++)
      {if (i!=0 and i!=2 and i!=3)  // bukan terbit dan bukan dhuha
          {if(floatnow >= sholatT[i])
              {SholatNow = i;
                if(!azzan and (floatnow > sholatT[i]) and (floatnow < (sholatT[i]+0.03/60))) 
